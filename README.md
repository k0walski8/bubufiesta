...
- **Full Metadata Embedding**: Complete ID3 tags, lyrics, and embedded cover art
...
### Custom Fork Image (GHCR)

If you're running your own fork and want a fixed image name for TrueNAS, this repo is preconfigured to publish:

- `ghcr.io/k0walski8/bubufiesta:latest`
- `ghcr.io/k0walski8/bubufiesta:dev`
- version tags when pushing `v*` tags

The Docker workflow runs automatically on pushes to `main`, `master`, and `dev`.

For TrueNAS custom apps, use:

- **Image**: `ghcr.io/k0walski8/bubufiesta`
- **Tag**: `latest` (or a version tag)
- **Container port**: `8080` (map host port as desired, e.g. `5274`)

If the package is private, configure GHCR credentials in TrueNAS. If public, no registry secret is needed.
